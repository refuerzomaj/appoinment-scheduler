...
private NotificationService notificationService;
            ...
            public String finishAppointment(@RequestParam("appointmentId") int appointmentId, Model model, @AuthenticationPrincipal CustomUserDetails currentUser) {
                // Retrieve the appointment by its ID and set its status to "FINISHED."
                Appointment appointment = appointmentService.getAppointmentById(appointmentId);
                appointment.setStatus(AppointmentStatus.FINISHED);
                appointmentService.updateAppointment(appointment);

                // Send a notification using the injected notificationService
                notificationService.newAppointmentFinishedNotification(appointment, true);

                // Send an email to the customer or provider (you'll need to implement this)
                sendFinishAppointmentEmail(appointment, currentUser);

                // Redirect to the appropriate page after finishing the appointment.
                return "redirect:/appointments/all"; // Adjust the URL as needed.
            }

        private void sendFinishAppointmentEmail(Appointment appointment, CustomUserDetails currentUser) {
            // Implement the logic to send an email to the customer or provider.
            // You can use your email service or framework of choice to send emails.
        }
